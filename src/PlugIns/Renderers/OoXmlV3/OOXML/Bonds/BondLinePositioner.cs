    ...
    public class BondLinePositioner
    {
        ...
        public void CreateLines(Bond bond)
        {
            ...
            switch (bond.Order)
            {
                case Bond.OrderZero:
                case "unknown":
                    m_BondLines.Add(new BondLine(bondStart, bondEnd, BondLineStyle.Dotted, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                    break;

                case Bond.OrderPartial01:
                    m_BondLines.Add(new BondLine(bondStart, bondEnd, BondLineStyle.Dashed, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                    break;
                ...
                case Bond.OrderSingle:
                    switch (bond.Stereo)
                    {
                        case BondStereo.None:
                            m_BondLines.Add(new BondLine(bondStart, bondEnd, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                            break;

                        case BondStereo.Hatch:
                            m_BondLines.Add(new BondLine(bondStart, bondEnd, BondLineStyle.Hatch, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                            break;

                        case BondStereo.Wedge:
                            m_BondLines.Add(new BondLine(bondStart, bondEnd, BondLineStyle.Wedge, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                            break;

                        case BondStereo.Indeterminate:
                            m_BondLines.Add(new BondLine(bondStart, bondEnd, BondLineStyle.Wavy, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                            break;
                ...
                case Bond.OrderAromatic:
                    BondLine a = new BondLine(bondStart, bondEnd, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id);
                ...
                case Bond.OrderDouble:
                    ...
                    {
                        // Crossed lines
                        BondLine d = new BondLine(bondStart, bondEnd, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id);
                        BondLine d1 = d.GetParallel(-(BondOffset() / 2));
                        BondLine d2 = d.GetParallel(BondOffset() / 2);
                        m_BondLines.Add(new BondLine(new Point(d1.Start.X, d1.Start.Y), new Point(d2.End.X, d2.End.Y), BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                        m_BondLines.Add(new BondLine(new Point(d2.Start.X, d2.Start.Y), new Point(d1.End.X, d1.End.Y), BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                    }
                    else
                    {
                        bool shifted = false;
                        ...
                        else
                        {
                            ...
                            switch (bond.Placement)
                            {
                                case BondDirection.Anticlockwise:
                                    BondLine da = new BondLine(bondStart, bondEnd, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id);
                                    m_BondLines.Add(da);
                                    ...
                                    if (bond.PrimaryRing != null)
                                    {
                                        ...
                                        m_BondLines.Add(new BondLine(outIntersectP1, outIntersectP2, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                                    }
                                    else
                                    {
                                        CoordinateTool.AdjustLineAboutMidpoint(ref startPointa, ref endPointa, -(BondOffset() / 1.75));
                                        m_BondLines.Add(new BondLine(startPointa, endPointa, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                                    }
                                    break;

                                case BondDirection.Clockwise:
                                    BondLine dc = new BondLine(bondStart, bondEnd, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id);
                                    m_BondLines.Add(dc);
                                    ...
                                    else
                                    {
                                        ...
                                        m_BondLines.Add(new BondLine(startPointc, endPointc, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                                    }
                                    break;
                                ...
                                default:
                                    ...
                                    {
                                        case BondStereo.Cis:
                                            BondLine dcc = new BondLine(bondStart, bondEnd, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id);
                                            m_BondLines.Add(dcc);
                                            ...
                                            m_BondLines.Add(new BondLine(startPointn, endPointn, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                                            break;

                                        case BondStereo.Trans:
                                            BondLine dtt = new BondLine(bondStart, bondEnd, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id);
                                            m_BondLines.Add(dtt);
                                            ...
                                            m_BondLines.Add(new BondLine(startPointt, endPointt, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id));
                                            break;

                                        default:
                                            BondLine dp = new BondLine(bondStart, bondEnd, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id);
                ...
                case "T":
                    // Draw main bond line
                    BondLine t = new BondLine(bondStart, bondEnd, BondLineStyle.Solid, bond.Id, bond.Parent.Id, bond.StartAtom.Id, bond.EndAtom.Id);
                    m_BondLines.Add(t);
